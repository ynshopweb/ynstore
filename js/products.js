...
export function setupProductsSnapshot() {
    ...
    onSnapshot(productsCol, (snapshot) => {
        if (snapshot.empty) {
            ...
            renderLandingPage();
            if(state.viewMode === 'admin') renderAdminProductsTable();
        }
    }, (err) => {
...
function renderLandingPage() {
    ...
    if(categoriesGrid) {
        categoriesGrid.innerHTML = categories.map(c => `
            <div onclick="filterByCategory('${c.name}')" class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition text-center cursor-pointer group">
    ...
    if(bestsellerGrid) {
        ...
        bestsellerGrid.innerHTML = bests.map(p => createProductCardHTML(p)).join('');
    }
}
...
function createProductCardHTML(product) {
    return `
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col group">
            <div class="relative overflow-hidden aspect-square bg-slate-100 cursor-pointer" onclick="openProductDetail('${product.id}')">
            ...
            <div class="p-3.5 flex-1 flex flex-col justify-between space-y-2">
                <div>
                    ...
                    <h4 onclick="openProductDetail('${product.id}')" class="font-bold text-xs text-slate-800 line-clamp-2 hover:text-brand-600 cursor-pointer transition">${product.name}</h4>
                </div>
                <div>
                    ...
                    <button onclick="addToCart('${product.id}')" class="w-full py-2 bg-brand-50 hover:bg-brand-600 text-brand-700 hover:text-white font-bold text-xs rounded-xl transition flex items-center justify-center gap-1">
...
function renderFilterOptions() {
    ...
    if (brandContainer) {
        brandContainer.innerHTML = brands.map(b => `
            <label class="flex items-center gap-2 text-xs text-slate-700 cursor-pointer">
                <input type="radio" name="filter-brand" value="${b}" ${state.activeFilters.brand === b ? 'checked' : ''} onchange="window.state.activeFilters.brand='${b}'; applyProductFilters();" class="accent-brand-600">
    ...
    if (categoryContainer) {
        categoryContainer.innerHTML = categories.map(c => `
            <label class="flex items-center gap-2 text-xs text-slate-700 cursor-pointer">
                <input type="radio" name="filter-category" value="${c}" ${state.activeFilters.category === c ? 'checked' : ''} onchange="window.state.activeFilters.category='${c}'; applyProductFilters();" class="accent-brand-600">
    ...
    card.innerHTML = `
        ...
        <div class="p-6 flex flex-col justify-between space-y-4">
            ...
            <div>
                ...
                <button onclick="addToCart('${p.id}'); document.getElementById('product-detail-modal').classList.add('hidden')" class="w-full py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs rounded-xl shadow transition flex items-center justify-center gap-2">
