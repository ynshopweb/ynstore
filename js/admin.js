...
function renderAdminOrdersTable() {
    ...
    tbody.innerHTML = state.orders.map(o => `
        <tr>
            ...
            <td class="p-3.5">
                ${o.proofImage ? `<button onclick="viewProofModal('${o.proofImage}')" class="px-2 py-1 bg-slate-700 text-slate-200 rounded text-[10px]"><i class="fa-solid fa-image me-1"></i> Lihat Foto</button>` : '<span class="text-slate-500 italic">Belum upload</span>'}
            </td>
            <td class="p-3.5"><span class="px-2 py-0.5 rounded text-[10px] font-bold ${getStatusBadgeColor(o.status)}">${o.status}</span></td>
            <td class="p-3.5 text-right space-x-1">
                <button onclick="updateOrderStatus('${o.orderId}', 'Diverifikasi & Dikemas')" class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] rounded">Verifikasi</button>
                <button onclick="updateOrderStatus('${o.orderId}', 'Siap Diambil di Toko')" class="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] rounded">Siap Ambil</button>
                <button onclick="updateOrderStatus('${o.orderId}', 'Selesai')" class="px-2 py-1 bg-slate-700 hover:bg-slate-600 text-white font-bold text-[10px] rounded">Selesai</button>
            </td>
        </tr>
    `).join('');
}
...
function renderAdminProductsTable() {
    ...
    tbody.innerHTML = state.products.map(p => `
        <tr>
            ...
            <td class="p-3.5 text-right space-x-1">
                <button onclick='editProduct(${JSON.stringify(p.id)})' class="px-2 py-1 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10px] rounded"><i class="fa-solid fa-pen"></i></button>
                <button onclick="deleteProduct('${p.id}')" class="px-2 py-1 bg-rose-600 hover:bg-rose-700 text-white font-bold text-[10px] rounded"><i class="fa-solid fa-trash"></i></button>
            </td>
        </tr>
    `).join('');
}
